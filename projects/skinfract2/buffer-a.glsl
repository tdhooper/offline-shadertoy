...
Material shadeModel(Model model, inout vec3 nor) {
    ...
    if (model.id == 1)
        return Material(skin, .15, .3, false);
...
//    pR(p.yz, .75);
    ...
    float l = 0.;
    float len = length(p) * spaceAnimFreq;

    float phase = time * PI * 2. + len * -50.0;
    ...
    vec3 anim = rotPhase + sin(phase + animPhase) * animAmp;
...
vec3 sunPos = normalize(vec3(-.6,.3,-.0)) * 100.;
...
vec4 draw(vec2 fragCoord, int frame) {
    ...
    for (int bounce = 0; bounce < MAX_BOUNCE; bounce++) {
        ...
        if (doSSS) {
            ...
            seed = hash22(seed);
            hit = walkOnSpheres(origin, nor, .025, seed);
            nor = calcNormal(hit.pos);
