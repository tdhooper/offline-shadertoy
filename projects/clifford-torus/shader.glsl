...
Model map(vec3 p) {
    ...
    pMod3(p, vec3(.05));
    d = length(p) - .03;
    float td = fTorus4(z4, 1., 1.);
    td = abs(td) - .001;
    d = max(td, - d);
    // d = td;
    model = Model(d, vec2(0), 0);
...
vec3 render(Hit hit){
    ...
    if ( ! hit.isBackground) {
        ...
        // col = vec3(col.b);
        col = mix(col, bg, clamp(0., 1., smoothstep(
            length(camPos * .25),
            length(camPos * 2.5),
            length(camPos - hit.pos)
        )));
    }
