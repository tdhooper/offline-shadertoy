...
Material shadeModel(Model model, inout vec3 nor) {
    ...
    blend = smoothstep(-.2, .6, model.albedo.y);

    //blend = step(model.albedo.y, .005);

    //blend = smoothstep(-1.75, -.0, model.albedo.x);
    skin += mix(vec3(-.6,.0,.15) * .2, vec3(.4,-.03,-.05) * .3, blend);
   ...
   // return Material(vec3(.8), .0, .0, false);
    ...
    //if (step(model.albedo.y, .0005) < 0.5) {
        return Material(skin, .15, .3, true);
    //} else {
    //    return Material(vec3(1), 1., 1., false);
    //}
    ...
    //col = mix(vec3(1), spectrum(model.albedo.y * .5 + .2), .75);// * mix(1., 3., smoothstep(0., .4, model.albedo.y));
    
    //blend = smoothstep(.0, -3., model.albedo.x);


   
    return Material(col, spec, rough, true);
}
...
vec3 sunPos = normalize(vec3(1.,-.25,-1.)) * 100.;
...
vec4 draw(vec2 fragCoord, int frame) {
    ...
    for (int bounce = 0; bounce < MAX_BOUNCE; bounce++) {
        ...
        if (doSSS) {
            ...
            seed = hash22(seed);
            hit = walkOnSpheres(origin, nor, .0125, seed);
            nor = calcNormal(hit.pos);

            float extinctionDist = distance(origin, hit.pos) * 5.;
