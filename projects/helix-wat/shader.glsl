...
void addPipe(inout float d, vec3 p, float scale, float tt) {
    ...
    float sz = mix(guiLead * 2., .5, separate);
    part = fBox2(p.yz, vec2(sz, .5) - .5, round);
    part -= .5;

    sz = .5;
    p.x += iGlobalTime;
    pMod1(p.x, sz * 2.5);
    float inner = fBox(p, vec3(sz));
    inner = length(p) - sz;
    // part = inner;

    // part = max(part-6., -part);
...
Model map(vec3 p) {
    ...
    // p.y *= -1.;
    // float a = pModSingle1(p.y, 3.);
    // p.z += a * .7;

    float f = pMod1(p.z, 1.2);

    float pipe = length(p.yz) - .5;

    // vec3 pl = cartToPolar(p);
    // p = pl.yxz;
    // p.z -= 1.6;
    ...
    float scaleB = 1.;
    scaleB *= pModHelix(p, guiLead, guiInnerRatio);

    float blend = sin(.5 * p.x + mod(iGlobalTime, 1.) * PI * 2.) * .5 + .5;
    // blend = smoothstep(.0, .75, blend);
    // blend = 0.;

    p.x *= -1.;
    //float t1 = unzip(p + vec3(offset,0,0), tt, true);
    // blend *= .3;
    blend += .01;
    addPipe(d, p, scaleB, blend);
...
/*
    pModSpiral(p, 1., sep, offset);

    float blend = p.x + iGlobalTime;
    blend = smoothstep(.0, .75, blend);

    float h = mix(sep * .5, sep * .25, blend);

    part = fBox2(p.yz, vec2(h, thick * .5));
    part = mix(part, length(p.yz) - thick * .5, 1.);
    
    d = mix(pipe, part, blend);
...
const float MAX_TRACE_DISTANCE = 15.; // max trace distance
...
// debug = true;
    ...
    for( int i=0; i < iter; i++ ){
        ...
        h = abs(m.dist*4.);
        t += max(INTERSECTION_PRECISION, h * FUDGE_FACTORR);
   ...
   // color *= 1. + (hash(vec3(p, t)) * 2. - 1.) * .4;
    // vec3 pos = ro+rd*t;
    //color = vec3(dist * .01);
    //color = mix(color, vec3(0), smoothstep(MAX_TRACE_DISTANCE, MAX_TRACE_DISTANCE, t));
