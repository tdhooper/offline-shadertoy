...
void pModHelix(inout vec3 p, float lead, float radius) {
    ...
    // x pos on helix
    // get distance
    // at 0 lead, d = radius * PI * 2
    // at 0 radius, d = lead
    // at radius r, lead l, d = 

    // when x is lead, d is length(vec2(radius * 2 * PI, lead)) 
...
Model map(vec3 p) {
    ...
    float lead1 = mix(.001, 20., pow(guiLevel1Lead, 2.));
    float radius1 = mix(0., 4., guiLevel1Radius);
    ...
    float len = 2.;
    float offset = 0.;
    float bumps = guiLevel2Lead * 10.;

    p = p.zxy;
    
    
    float rad = mix(.2, .6, cos(p.y * PI / len * bumps) * .5 + .5);
    p.y -= len;
    d = fCapsule(p, rad, len);
...
const float MAX_TRACE_DISTANCE = 200.;
const float INTERSECTION_PRECISION = .001;
const int NUM_OF_TRACE_STEPS = 2000;
const float FUDGE_FACTOR = .8;
