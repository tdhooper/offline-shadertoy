...
//  pR(p.yx, anm * PI * .5);
  ...
  float mask = 1.;
  if (length(vec2(c.x, length(c.yz) - rad / sz)) < 1.) {
    mask = 0.;
  }
  ...
  if (mask > 0.) {
  //  d = min(d, length(p.xz) - .01);
 //   d = min(d, length(p.yz) - .01);
//    d = min(d, length(p.yz) - .01);
  }  
...
void main() {
  ...
  vec3 bb = vec3(0,1,0);
  mat3 camMat = mat3(
    normalize(cross(dir, bb)),
    normalize(cross(dir, normalize(cross(dir, bb)))),
    dir
  );
  ...
  float fpd = .5;
  vec3 fp = eye + dir * fpd;
  eye = eye + camMat * vec3(rndunit2(seed), 0.) * .05;
  dir = normalize(fp - eye);
  ...
  // color *= modelColor;
  color = mix(color, vec3(0), pow(smoothstep(1.5, MAX_DIST, rayLength), .5));
