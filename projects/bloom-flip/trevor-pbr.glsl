...
vec4 worldPos()
{
    float depthSample = texelFetch(uDepthBuffer, ivec2(gl_FragCoord.xy), 0).x;
    if(depthSample==1.0)
        discard;
    vec2 cc = (gl_FragCoord.xy * 2.0 - uResolution) / uResolution.y;
    vec3 frustumRayDirection = normalize(vec3(uFrustumTop * cc, -1.0));
    float frustumRayDirectionZ = frustumRayDirection.z;
    float ndcDepth = (depthSample * 2.0 - gl_DepthRange.near - gl_DepthRange.far) / gl_DepthRange.diff;
    float totalDistance = -uP[3].z / ((ndcDepth + uP[2].z) * frustumRayDirectionZ);
    return uC * vec4(frustumRayDirection * totalDistance, 1.0);
}
...
void main()
{
    // Deferered buffer unpack
    vWorldPos = worldPos().xyz;
    vec3 V = normalize(uC[3].xyz - vWorldPos);
    vec3 N = oct_to_float32x3(unorm8x3_to_snorm12x2(texelFetch(uN, ivec2(gl_FragCoord.xy), 0).xyz));
    vec4 aa = texelFetch(uAlbedoRoughness, ivec2(gl_FragCoord.xy), 0);
    vec3 F0 = texelFetch(uF0, ivec2(gl_FragCoord.xy), 0).xyz;
    LightData info = LightData(Material(aa.xyz, aa.w, F0), N, saturate(dot(V, N)), reflect(-V, N), V);
