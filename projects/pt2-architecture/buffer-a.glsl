...
float lightRadius = .125;
vec3 lightColor = vec3(8.10,6.00,4.20) * 4.;
...
Model map(vec3 p) {
    ...
    float d, dd;
    int id = 1;    
    vec3 albedo = vec3(1.000,0.616,0.180);
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        //pR(p.xz, -.5);
        float d2 = sdBoundingBox(p, vec3(.8,.3-(1.-c) * .2,.5+(1.-c) * .2), .05 + (1.-c));
        //d2 = min(d2, sdBoundingBox(p, vec3(1.05,.35,.45), .025));
       // d2 = min(d2, fBox(p, vec3(1.,.3,1.5) - .1));
        //pR(p.xz, .5);
    ...
    if (dd < d) {
        ...
        //col = mix(vec3(1,1,0), vec3(0,1,1), step(p.x,p.z)) * 50.;
    }
