...
float animSubdivisions1() {
    return 4.5;
    return mix(4.4, 5.4, cos(time * PI * 2.) * .5 + .5);
}

HexSpec animHex1(vec3 hexCenter, float subdivisions) {
    HexSpec spec = newHexSpec(subdivisions);
    
    float offset = time * 2. * 3. * PI;
    offset -= subdivisions;
    float blend = dot(hexCenter, pca);
    //float freq = (cos(time * PI) * .5 + .5);
    blend = cos(blend * 50. + offset) * .5 + .5;
    spec.height = mix(1.5, 2., blend);
...
Model hexModel(
    ...
    float edgeAnim = abs(time - 1.);
    edgeAnim = smoothstep(.3, .7, edgeAnim);
    float edge = mix(-.02, -.17, edgeAnim);
    edgeBlend = 1. - step(edgeDist, edge);
