...
vec3 camPath(float t) {
    ...
    vec3 start = vec3(0);
    vec3 end = triV.c * (stepMove * (1./stepScale));

    float r = smoothstep(.1, .5, t) - smoothstep(.5, .9, t);

    start += triV.b * 1.5 * r;
    end += triV.b * 1.5 * r;

    vec3 x = normalize(cross(start - end, triV.a));
    vec3 y = normalize(cross(start - end, x));
