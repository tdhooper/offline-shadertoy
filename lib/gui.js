...
var Control = function(gui, globalState, key, conf) {
    conf = conf.slice();
    this.min = conf[0];
    this.max = conf[1];
    this.globalState = globalState;
    this.key = key;
    this.globalState[this.key] = conf.pop();
    conf.unshift(this.globalState, this.key); // add global state, and key to start
...
Control.prototype.value = function() {
    return this.globalState[this.key];
};
...
GUI.prototype.initControllers = function(
    ...
    Object.keys(config).forEach(function(key) {
        ...
        var control = new Control(gui, state, key, conf);
        if (midiConf) {
