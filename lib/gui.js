...
GUI.prototype.linkMidiControl = function(controller, controlConf, midiConf) {
    ...
    var midiController = slotConfig[0];
    var channel = slotConfig[1];
...
GUI.prototype.addMidiChangeListener = function(config, listener) {
    this.midiInput.then(function(input) {
        var channel = config[0];
        var midiController= config[1];
