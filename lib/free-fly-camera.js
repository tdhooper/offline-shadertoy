...
const Camera = function(options) {
  ...
  this.simPositionTemp = vec3.clone(this.position);
  this.simPositionPrev = vec3.clone(this.position);
  ...
  this.accumulator = 0;
  this.simDt = 1;
  this.previousState = vec3.create();
};
...
Camera.prototype.control = function(dt, move, mouse, prevMouse, shift) {
  ...
  let positionSpeed = this.positionSpeed / 200000;
...
Camera.prototype.integrate2 = function(dt) {

  vec3.copy(this.simPositionTemp, this.simPosition);
  vec3.scale(this.simPosition, this.simPosition, 2);
  vec3.sub(this.simPosition, this.simPosition, this.simPositionPrev);
  vec3.scale(this.accellerationDt, this.accelleration, dt * dt);
  vec3.add(this.simPosition, this.simPosition, this.accellerationDt);
  vec3.copy(this.simPositionPrev, this.simPositionTemp);
...
Camera.prototype.simulate = function() {
 ...
 // console.log((new Array(Math.floor(frameTime / 2))).fill('X').join(''));
 // console.log(this.accelleration);
  //console.log((performance.now() - newTime) > frameTime);
}
...
Camera.prototype.move = function(dir) {
  ...
  vec3.scale(this.drag, this.velocity, -.005);
