...
float gmTransform(int index, inout vec3 p) {
    float scale = NON_ADJUSTABLE_gmTransform(p, gizmoAdjustment[index].t, gizmoAdjustment[index].r, gizmoAdjustment[index].s);
    return scale;
}
`;
  source = source.slice(0, insertIndex) + insert + source.slice(insertIndex);
...
void main() {
  GIZMO_MAIN();
  vec3 position = texelFetch(evalGizmoPositions, ivec2(gl_FragCoord.x, gl_FragCoord.y), 0).rgb;
  GIZMO_ARGUMENTS_SET[0] = false;
  GIZMO_MAP(position);

  if (gl_FragCoord.x < 1.) {
    fragColor = vec4(GIZMO_ARGUMENTS[0].t, 0);
  } else if (gl_FragCoord.x < 2.) {
    fragColor = GIZMO_ARGUMENTS[0].r;
  } else if (gl_FragCoord.x < 3.) {
    fragColor = vec4(GIZMO_ARGUMENTS[0].s, 0);
  } else {
    fragColor = vec4(GIZMO_ARGUMENTS[0].p, 0);
  }
