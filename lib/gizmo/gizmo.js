...
export default class Gizmo {
  ...
  async initialise(camera, mouse) {
    ...
    document.addEventListener('pointerdown', this._enable.bind(this));
  ...
  _updateThreeCamera = (state) => {
    ...
    this.threeCamera.aspect = state.cameraProjection[1 * 4 + 1] / state.cameraProjection[0 * 4 + 0];
    this.threeCamera.fov = THREE.MathUtils.radToDeg(Math.atan(1 / (state.cameraProjection[1 * 4 + 1]) * 2));
