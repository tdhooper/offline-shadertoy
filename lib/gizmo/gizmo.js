...
export default class Gizmo {
  ...
  initialise(camera, mouse, renderNodes, uniforms) {
    ...
    for (let i = 0; i < this.gizmoCount; i++) {
      ...
      gizmoControl.addEventListener('mouseDown', (pointer) => {
  ...
  _setupThree() {
    ...
    // Important to attach this before TransformControls attaches its events
    // so we can move it into place first 
  ...
  render() {
    if ( ! this.shadersHaveGizmoFunction || ! this.shadersHaveMapFunction || ! this.enabled) {
      return;
    }
