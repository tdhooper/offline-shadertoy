...
export default class Gizmo {
  ...
  async start() {
    this.gizmoCount = await this.worker.gizmoCount();
    this.shaderFiles = await this.worker.shaderFiles();
  }
  
  async initialise(camera, mouse) {

    let gizmoWorkerInitialised = await this.worker.gizmoInitialise();
  ...
  async _updateGizmos(state) {
    ...
    let results = await this.worker.gizmoFindTransforms(state);
