...
export default class Gizmo {
  ...
  constructor(gizmoWorker, container) {
    this.gizmoWorker = gizmoWorker;
    this.container = container;
  }

  async start() {
    this.gizmoCount = this.gizmoWorker.gizmoCount;
    this.shaderFiles = this.gizmoWorker.shaderFiles;
  }
  
  async initialise(camera, mouse) {

    let gizmoWorkerInitialised = this.gizmoWorker.initialise();

    if ( ! gizmoWorkerInitialised) {
      return;
    }
  ...
  async _updateGizmos(state) {
    ...
    let results = this.gizmoWorker.findTransforms(state);
