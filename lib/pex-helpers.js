...
const wrapConst = (wrap) => {
  let wrapMap = {
    clamp: ctx.Wrap.ClampToEdge,
    repeat: ctx.Wrap.Repeat,
    mirror: ctx.Wrap.MirroredRepeat
  }
  return wrap in wrapMap ? wrapMap[wrap] : null;
};

const filterConst = (filter) => {
  if ( ! ctx.capabilities.textureFloatLinear) {
    return ctx.Filter.Nearest;
  }
  let filterMap = {
  ...
  return filter in filterMap ? filterMap[filter] : null;
};
