...
class Gizmo {
  ...
  _preprocessShader(shader) {
    ...
    shader.glsl = `
      ...
      uniform vec3 gizmoAdjustmentT;
      uniform vec4 gizmoAdjustmentR;
      uniform vec3 gizmoAdjustmentS;
      ...
      float egTransform(inout vec3 p, vec3 t, vec4 r, vec3 s) {
          ...
          float scale;
          if (gizmoAdjust) {
              scale = gTransform(p, gizmoAdjustmentT, gizmoAdjustmentR, gizmoAdjustmentS);
          } else {
      ...
      float egTransform(inout vec3 p) {
          if ( ! GIZMO_SET) GIZMO_LOCAL_P = p;
          if ( ! GIZMO_SET) GIZMO_LOCAL_P2 = p;
          float scale = gTransform(p, gizmoAdjustmentT, gizmoAdjustmentR, gizmoAdjustmentS);
          GIZMO_SET = true;
          return scale;
      }
    ` + shader.glsl;
  ...
  initialise(camera, mouse, renderNodes, uniforms) {
    ...
    uniforms.gizmoAdjustmentT = regl.prop('gizmoAdjustmentT');
    uniforms.gizmoAdjustmentR = regl.prop('gizmoAdjustmentR');
    uniforms.gizmoAdjustmentS = regl.prop('gizmoAdjustmentS');
  ...
  _save() {
    ...
    Object.assign(data, this._adjustmentObj(this.controlObject));
  ...
  toState() {
    ...
    let state = {
      gizmoAdjust: this.ready,
    };
    
    Object.assign(state, this.adjustmentObj);
  ...
  update(state) {
    ...
    if ( ! this.ready) {
      ...
      mat4.invert(workingMat, this.transforms.before);
      ...
      mat4.invert(workingMat, this.transforms.after);
      mat4.multiply(workingMat, this.transforms.before, workingMat); // Remove the parent matrix from it  
      threeWorkingMat.fromArray(workingMat);
      threeWorkingMat.decompose(this.controlObject.position, this.controlObject.quaternion, this.controlObject.scale);

      // Force matrix update so we can grab it for toState
